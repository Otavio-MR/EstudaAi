...
const createStudying = async (req,res) => {
    try{
        ...
        const existing = await Studying.findOne({nome});
        if(existing){
            return res.status(400).json({message:'Não foi possível favoritar a matéria'})
        }
        const studying = await Studying.create({nome, concluida});
